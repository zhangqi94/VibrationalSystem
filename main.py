...
parser.add_argument("--w2_indices", type=float, nargs="+", default=[0], help="w^2")
parser.add_argument("--lam", type=float, nargs="+", default=[0], help="lambda")

### vibrational self-consistent field (VSCF)
parser.add_argument("--vscf_base", action='store_true', help="use VSCF base wavefunction")
parser.add_argument("--vscf_nlev", type=int, default=10, help="VSCF number of levels")
...
vscf_base, vscf_nlev = args.vscf_base, args.vscf_nlev
...
C_vscf = 0.0
if vscf_base:
    from src.vscf import solve_vscf
    C_vscf, _ = solve_vscf(n, w_indices, k2, k3, k4, orb_state[0], Nlev=vscf_nlev, iters_vscf=10, print_level=2)
    C_vscf = jnp.array(C_vscf, dtype=jnp.float64)
    jnp.set_printoptions(precision=3, suppress=True, formatter={'float': '{:0.6f}'.format})
...
elif flow_type == "RNVPsingle":
    flow_str = ("_rnvp1_%d" % flow_depth) + ("_mlp_%d_%d" % (mlp_width, mlp_depth))

if vscf_base == True:
    vscf_str = "_vscf_%d" % vscf_nlev
else:
    vscf_str = ""
...
for ii in range(epoch_finished+1, epoch+1):
    ...
    if print_levels:
        datas_acc = replicate({"E_mean": 0., "E2_mean": 0., 
                            ...
                            "Levels": jnp.zeros((real_num_orb_in_state_indices))}, 
                            num_devices)
    else:
        datas_acc = replicate({"E_mean": 0., "E2_mean": 0., 
                            "K_mean": 0., "K2_mean": 0., 
    ...
    if mode in ["NDCO", "TOY2", "TOY3"]:
        ...
        if print_levels:
            print("iter: %05d" % ii,
                " E: %.2f (%.2f)  K: %.2f (%.2f)  V: %.2f (%.2f)" % (LE, LE_std, LK, LK_std, LV, LV_std),
                " ac: %.4f  dx: %.4f  dt: %.3f" % (accept_rate, mc_stddev, t2-t1),
                " levels:  ", "%.2f  "*real_num_orb_in_state_indices % tuple(Levels), flush=True)
        else:
            print("iter: %05d" % ii,
                " E: %.2f (%.2f)  K: %.2f (%.2f)  V: %.2f (%.2f)" % (LE, LE_std, LK, LK_std, LV, LV_std),
                " ac: %.4f  dx: %.4f  dt: %.3f" % (accept_rate, mc_stddev, t2-t1), flush=True)
    ...
    if print_levels:
        f.write("  %.4f"*real_num_orb_in_state_indices % tuple(Levels))
    ...
    if ii % ckpt_epochs == 0:
        ckpt = {"keys": keys, "x": x, "opt_state": opt_state,
                ...
                "vscf_base": vscf_base, "vscf_nlev": vscf_nlev, "C_vscf": C_vscf,
                "flow_type": flow_type,
