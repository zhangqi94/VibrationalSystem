...
def get_maskflow(key, nvp_depth, event_size):
    ...
    maskflow = np.ones((nvp_depth, event_size))
    for ii in range(nvp_depth):
        maskflow[ii, ii%event_size] = 0
    maskflow = maskflow.astype(bool)
    return maskflow

# def get_maskflow(key, nvp_depth, event_size):
#     import numpy as np
    
#     maskflow = np.zeros((nvp_depth, event_size))
#     for ii in range(nvp_depth):
#         maskflow[ii, ii%event_size] = 1
#     maskflow = maskflow.astype(bool)
#     return maskflow
...
if __name__ == "__main__":
    ...
    maskflow = jnp.array(get_maskflow(key, nvp_depth, n*dim))
    print("maskflow", maskflow)

    x_flatten = jax.random.normal(key, (n, dim)).reshape(-1)
    x1 = jnp.where(maskflow[0], x_flatten, 0)
    x2 = jnp.where(maskflow[0], 0, x_flatten)
    print("x1:", x1)
    print("x2:", x2)
