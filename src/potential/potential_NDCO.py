...
####################################################################################
def get_potential_energy_NDCO(D=64):
    """
    ...
    for i in range(D):
        for j in range(D):
            if i == j:
                ...
                Vij[i,j] = a * np.sqrt(w_indices[i] * w_indices[j])
    Vij = jnp.array(Vij)
...
####################################################################################
def calculate_exact_energy_NDCO(D=64, num_orb=80):
...
if __name__ == "__main__":
    ...
    #==== exact energy levels ====
    num_orb = 21
    orb_idx, orb_state, orb_Es, nu = calculate_exact_energy_NDCO(D=n, num_orb=num_orb)
    print("\n==== exact values ====")
    print("nu:", nu)
    import orbitals
