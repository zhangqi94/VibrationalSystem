...
def make_logpsi(flow, sp_orbitals, orb_state, w_indices):
    ...
    def logpsi(x, params, state_indices):
        ...
        # distrax RealNVP
        z_flatten, logjacdet = flow.apply(params, x.reshape(-1), 1)
        log_phi = logphi_base(sp_orbitals, orb_state[state_indices], w_indices, z_flatten.reshape(n, dim))
...
def make_logphi_logjacdet(flow, sp_orbitals, orb_state, w_indices):
    ...
    def logphi(x, params, state_indices):   
        n, dim = x.shape
        z_flatten, _ = flow.apply(params, x.reshape(-1), 1)
        log_phi = logphi_base(sp_orbitals, orb_state[state_indices], w_indices, z_flatten.reshape(n, dim))
        return jnp.stack([log_phi.real, log_phi.imag])

    def logjacdet(x, params):
        _, logjacdet = flow.apply(params, x.reshape(-1), 1)
